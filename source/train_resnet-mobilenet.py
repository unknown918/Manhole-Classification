...
def model_load(IMG_SHAPE=(224, 224, 3), class_num=5):
    ...
    # 对输入进行归一化处理
    normalized_input = tf.keras.layers.experimental.preprocessing.Rescaling(1. / 127.5, offset=-1)(input_layer)
...
def train(epochs):
    ...
    begin_time = time()
    # 加载数据集， 修改为你的数据集的路径
    train_ds, val_ds, class_names = data_load("../datasets/train",
                                              "../datasets/val",
                                              224, 224, 16)
    print(class_names)
    # 加载模型
    ...
    # 保存模型， 修改为你要保存的模型的名称
    model.save("../models/res_mobile.h5")
    # 记录结束时间
    end_time = time()
