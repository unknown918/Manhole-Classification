...
class Results(SimpleClass):
    ...
    def save_txt(self, txt_file, save_conf=False):
        """
        Save predictions into txt and xml file.
        ...
        if probs is not None:
            ...
            for j, d in enumerate(boxes):
                ...
                ET.SubElement(xml_object, 'name').text = self.names[c]
                ...
                xyxy = d.xyxy.cpu().numpy().tolist()[0]
                ET.SubElement(bndbox, 'xmin').text = str(int(xyxy[0]))  # Multiply by image width
                ET.SubElement(bndbox, 'ymin').text = str(int(xyxy[1]))  # Multiply by image height
                ET.SubElement(bndbox, 'xmax').text = str(int(xyxy[2]))  # Multiply by image width
                ET.SubElement(bndbox, 'ymax').text = str(int(xyxy[3]))  # Multiply by image height
        ...
        if texts:
            ...
            ET.SubElement(xml_root, 'folder').text = 'test'
            ET.SubElement(xml_root, 'filename').text = Path(txt_file).stem + '.jpg'
            ET.SubElement(xml_root, 'path').text = Path(txt_file).stem + '.jpg'
            source = ET.SubElement(xml_root, 'source')
            ET.SubElement(source, 'database').text = 'Unknown'
            size = ET.SubElement(xml_root, 'size')
            ET.SubElement(size, 'width').text = str(self.orig_shape[1])
            ET.SubElement(size, 'height').text = str(self.orig_shape[0])
            ET.SubElement(size, 'depth').text = str(3)
            ET.SubElement(xml_root, 'segmented').text = str(0)
            for xml_object in xml_objects:
                xml_root.append(xml_object)
